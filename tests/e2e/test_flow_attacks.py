"""Flow-based attack tests: DDoS and brute-force flow records trigger XGBoost
detection and reaction escalation RATE_LIMIT -> BLOCK.
"""
import aio_pika
import pytest
...
# DDoSReactionService / BruteForceReactionService both escalate to BLOCK after
# ESCALATION_THRESHOLD detections (matching EscalatingIpReactionService).
ESCALATION_THRESHOLD = 3


@pytest.mark.asyncio
@pytest.mark.parametrize(
    "source_ip,dest_ip,dest_port,features,n_records,detection_type",
    [
        pytest.param(
            "10.55.1.100", "10.0.0.1", 80, DDOS_FEATURES, 6, "DDOS",
            id="ddos",
        ),
        pytest.param(
            "10.66.1.100", "10.0.0.2", 22, BRUTE_FORCE_FEATURES, 10, "BRUTE_FORCE",
            id="brute_force",
        ),
    ],
)
async def test_flow_attack_detected_and_ip_blocked(
    rmq_connection, pg_conn, redis_client,
    source_ip, dest_ip, dest_port, features, n_records, detection_type,
):
    """
    Full pipeline (via log.raw): crafted flow records -> normalized_flow -> detection
    ...
    CIC-IDS-2017 feature names at extreme attack-characteristic values so the
    ...
    # XGBoost must produce enough detections to cross the escalation threshold.
    async def enough_detections():
        n = await pg_conn.fetchval(
            "SELECT COUNT(*) FROM analysis.detection_results WHERE detection_type = $1",
            detection_type,
        )
