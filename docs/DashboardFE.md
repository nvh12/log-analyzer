...
-   **Apply changes button**: Appears when any pending changes exist. On click it fires exactly two API calls in parallel, both against the Dashboard service (`api.js`), not Simulation directly — `PUT /api/reactions/whitelist` (replace the entire whitelist; Dashboard's backend proxies this one through to Simulation's `PUT /admin/whitelist`) and `POST /api/reactions/blocks/lift` (lift all checked IPs; this is a direct Redis operation inside Dashboard — it never reaches Simulation) — then resets pending state and reloads live data.
