...
| Service Name | Base Docker Image | Network Namespace | Port Mapping (Host:Container) | Data Persistence / Volumes |
| :--- | :--- | :--- | :--- | :--- |
| **`rabbitmq`** | `rabbitmq:4.2.4-management` | `log-analyzer` | `127.0.0.1:5672:5672`<br>`127.0.0.1:15672:15672` | `rabbit-data:/var/lib/rabbitmq` |
| **`postgres-db`** | `postgres:17.5` | `log-analyzer` | `127.0.0.1:5432:5432` | `db-data:/var/lib/postgresql/data` |
| **`redis`** | `redis:7.4` | `log-analyzer` | `127.0.0.1:6379:6379` | `redis-data:/data` |
| **`minio`** | `minio/minio:RELEASE.2025-09-07...` | `log-analyzer` | `127.0.0.1:9000:9000`<br>`127.0.0.1:9001:9001` | `minio-data:/data` |
| **`log-processing`** | *Custom Java 21 build* | `log-analyzer` | `127.0.0.1:8080:8080` | N/A (Ephemeral) |
| **`log-analysis`** | *Custom Python 3.12 build* | `log-analyzer` | `127.0.0.1:8000:8000` | N/A (Ephemeral) |
| **`simulation`** | *Custom Python 3.12 build* | `log-analyzer` | `127.0.0.1:8001:8001` | N/A (Ephemeral) |
| **`reaction`** | *Custom Java 21 build* | `log-analyzer` | `127.0.0.1:8082:8080` | N/A (Ephemeral) |
| **`dashboard`** | *Custom Java 21 build* | `log-analyzer` | `127.0.0.1:8083:8080` | N/A (Ephemeral) |
| **`dashboard-fe`** | `nginx:1.27-alpine` | `log-analyzer` | `80:80`<br>`443:443` | `/etc/letsencrypt/` (SSL certificates) |

*Note: In Deploy Mode, all internal services bind exclusively to localhost (`127.0.0.1`) to ensure network isolation, leaving Nginx (`dashboard-fe`) as the single entrypoint for external clients.*
...
| `DOMAIN` | `dashboard-fe` (Nginx volumes) | Target domain to locate Let's Encrypt certificates. | Public |
