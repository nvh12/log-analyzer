...
Whitelist không còn do Dashboard/Reaction lưu trữ trực tiếp trong Redis của riêng chúng — quyền sở hữu `whitelist:ips` đã chuyển sang **Simulation service** (`presentation/routers/access_control_router.py`), nơi expose `GET/POST/DELETE/PUT /admin/whitelist` (gated bởi header `X-Admin-Key`). dashboard-fe **không** gọi trực tiếp các endpoint này — nó gọi `GET/PUT /api/reactions/whitelist` trên Dashboard (`ReactionController`), Dashboard mới là nơi proxy sang Simulation qua `SimulationWhitelistAdapter` (`WhitelistPort`); chỉ riêng các route điều khiển mô phỏng (`/simulate/start`, `/simulate/stop`, `/simulate/replay`, `/simulate/baseline`) mới đi qua Vite proxy `/simulate` trực tiếp tới Simulation. Reaction service không còn lưu whitelist nào cả; trước mỗi hành động chặn/giới hạn tốc độ trên một IP, nó gọi `WhitelistService.isWhitelisted(ip)`, được hiện thực bởi `SimulationWhitelistClient` — một lệnh gọi HTTP `GET /admin/whitelist` tới Simulation, fail-open (coi như không whitelist) nếu Simulation không phản hồi. Dashboard chỉ còn API gỡ chặn (`POST /api/reactions/blocks/lift`) cho các IP đang nằm trong `blocklist:ips`.
...
| WL-03 | `isWhitelisted()` khi Simulation không thể truy cập (lỗi server) | Simulation trả về `500` | Fail-open: trả về `false`, không ném exception | Đạt |
| WL-03b | `isWhitelisted()` khi Simulation trả về `403 Forbidden` | Simulation trả về `403` (vd `X-Admin-Key` sai/thiếu) | Fail-open: trả về `false`, không ném exception | Đạt |
...
| WL-06 | `isWhitelisted()` gọi lần 2 trong TTL cache | Lần đầu gọi với `1.2.3.4` (có trong response); lần 2 gọi với `5.6.7.8` trong cùng TTL | Chỉ 1 request HTTP thực tới Simulation; lần gọi thứ 2 phục vụ từ cache nội bộ (`5.6.7.8` → `false`, không có trong response đã cache) | Đạt |
...
**Thống kê:** 16 trường hợp kiểm thử đơn vị (9 cho leo thang phản ứng RX-01–RX-09 + 7 cho whitelist WL-01–WL-06, bao gồm WL-03b/WL-06 cho nhánh 403 và TTL cache của `SimulationWhitelistClient`) và 10 trường hợp kiểm thử tích hợp (5 RX-IT-01–RX-IT-05 + 5 WL-IT-01–WL-IT-05) = **26 trường hợp**.
...
| RES-06 | `DeadLetterConsumer`: `dropAuditRepository.recordDeadLetter()` ném lỗi, body parse được | Mock ném `RuntimeException("audit down")`; body = `{"id":"id-err"}` | Không lan truyền ngoại lệ ra ngoài `onDeadLetter()`; `failedLogRepository.save()` được gọi để requeue bản ghi vào Redis DLQ | Đạt |
| RES-06b | `DeadLetterConsumer`: `dropAuditRepository.recordDeadLetter()` ném lỗi, body không parse được | Mock ném `RuntimeException("audit down")`; body = `"not json"` | Không lan truyền ngoại lệ; `failedLogRepository` không được gọi (không thể requeue bản ghi không xác định được) | Đạt |
...
**Thống kê:** 13 trường hợp đơn vị mới (trong đó `DeadLetterConsumerTest` là file hoàn toàn mới với 7/7 trường hợp — bao gồm RES-06b xác nhận requeue vào `failedLogRepository` khi audit lỗi nhưng body parse được — trước đây `DeadLetterConsumer` không có kiểm thử đơn vị) + 2 trường hợp tích hợp đã có từ trước (`DeadLetterConsumerIT`, `DlqRetrySchedulerIT`) = **15 trường hợp**, tất cả đạt.
...
| DJ-02 | Chạy traffic bình thường | Cửa sổ có 1 log; lịch sử `[100.0, 101.0]`; bucket mùa `[(95.0, 1.0)]` | `execute()` nhận `seasonal_summaries=[(95.0, 1.0)]`; `update_history("traffic:history", [100.0, 101.0, 1.0], limit=60)` | Đạt |
...
|---|---|---|---|
| log-processing | 67 | 28 | **95** |
| log-analysis (Python) | 112 | 2 | **114** |
| reaction | 115 | 26 | **141** |
| dashboard | 34 | 47 | **81** |
| simulation (Python) | 204 | 4 | **208** |
| E2E (toàn hệ thống) | — | 8 | **8** |
| **Tổng cộng** | **532** | **107** | **647** |

Tất cả **647/647 trường hợp kiểm thử đều đạt** (passed). Số liệu trên được đếm trực tiếp từ số phương thức `@Test` (Java, có cộng các trường hợp kế thừa từ `EscalatingIpReactionServiceTestBase` qua `BruteForceReactionServiceTest`/`DDoSReactionServiceTest`) và `pytest --collect-only` (Python, phản ánh đúng số test case sau khi tính cả các hàm `@pytest.mark.parametrize`).
...
>
> **Cập nhật gần nhất**: bảng trên hiện phản ánh các thay đổi mới nhất chưa commit — `simulation/main.py`'s ownership-scoped baseline shutdown fix (kèm các trường hợp mới trong `simulation/tests/test_main.py`), DLQ audit path của reaction (`ReactionDeadLetterConsumer` + `reaction.dropped_reactions`, file mới `ReactionDeadLetterConsumerTest` 4 trường hợp, cùng `DetectionResultConsumerIT.ddosDetection_handlerThrows_messageRoutedToDlqNotLost`), và việc loại bỏ một số trường hợp dashboard trùng lặp/flaky (`DashboardApplicationTests` bị xoá hoàn toàn; `SystemControllerIT`, `DetectionControllerIT`, `ReactionResultListenerTest` được rút gọn).

Các bổ sung kiểm thử trong vòng rà soát gần nhất, theo từng dịch vụ:

- **log-processing**: thêm 12 trường hợp biên cho `LogProcessingService` (mã trạng thái HTTP 100/99/599/600, độ dài URL/IP/User-Agent tối đa và vượt ngưỡng, phương thức HTTP không xác định, referer rỗng vs `null`, sanitize giá trị Flow feature tràn số `Infinity`/`-Infinity` về `0.0`); 1 trường hợp cho `DlqRetryScheduler` (lỗi audit khi retry đã hết hạn không lan truyền); 2 trường hợp cho `RedisQueueService` (script Redis trả `null`, Redis ném lỗi khi enqueue); 2 trường hợp cho `RawLogConsumer` (enqueue ném lỗi → DLQ, `receivedAt=null` → tự gán `now()` và xử lý bình thường); 1 trường hợp cho `LogProcessingPoller` (bỏ qua dequeue khi hàng đợi executor vượt ngưỡng backpressure); và file mới `DeadLetterConsumerTest` (7 trường hợp cho việc trích xuất lý do/log id từ message DLQ và requeue vào `failedLogRepository` khi audit lỗi). Đồng thời gỡ bỏ `EventServiceImplTest` (trùng lặp hoàn toàn với `EventServiceImplIT` chạy trên RabbitMQ thực).
