...
The **log-analyzer** implementation for **UC1** specifically utilizes the **Ensemble Rule Engine** combining **EMA**, **Z-Score**, **IQR**, and **Seasonal Baseline (V2)** methods aggregated by **weighted-axis voting**. The system emits per-minute results to the `detection.results` stream with the full schema: `scores` (per-detector continuous scores), `flags` (per-detector booleans), `vote_count` (raw integer count, diagnostic), `weighted_votes` (continuous aggregate in [0, 3.0], the alert driver), `max_norm_score`, `severity` (5-level), and `alert` (binary). The chosen operating threshold $V_{\min}^{\star}$ and the per-detector $k_i$ values are persisted in the calibration JSON loaded by the FastAPI Detection service at startup.
