...
To optimize the **Detection microservice**, UC4 uses the same reduced feature set as UC2 (defined in `flow_feature_cols.json`), consisting of exactly **43 features**. This ensures that a single incoming flow record can be processed by both the DDoS and Brute Force models simultaneously without redundant feature extraction.
