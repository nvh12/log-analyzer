...
-   **UC1 - Traffic Spike Detection**: Executes every 60 seconds (`CRON_TRAFFIC`, cron `0 * * * * *`). Uses an ensemble of EMA, Z-Score, IQR, and Seasonal Baseline statistical models. Results are aggregated via a weighted-axis voting engine. The EMA component is carried continuously across ticks rather than re-seeded from each window's history: `TrafficUseCase.execute()` takes a `prev_ema` argument and returns the `updated_ema` float, which `DetectionJobRunner` persists via `HistoryPort.update_ema_state()`/reads via `get_ema_state()` so the next tick's EMA recursion stays on the same continuous curve the calibration notebook uses. Z-Score, IQR, and EMA each clamp against their own variance floor (`TRAFFIC_Z_SCORE_VARIANCE_FLOOR`, `TRAFFIC_IQR_VARIANCE_FLOOR`, `TRAFFIC_EMA_VARIANCE_FLOOR`, all 5.0 by default) instead of one shared floor, since the three detectors can see different natural noise scales on the same traffic; the Seasonal Baseline has its own separate `TRAFFIC_SEASONAL_SCALE_FLOOR`. The effective floor is `max(absolute_floor, *_VARIANCE_FLOOR_PCT * recent_baseline_mean)` — the absolute value alone is calibrated for low-volume traffic, so without the relative term, unusually steady traffic at a high baseline (e.g. a few-hundred-req/min baseline with low natural jitter) gets an ordinary few-percent fluctuation amplified past threshold once the absolute floor clamps the measured std. `TRAFFIC_MIN_HISTORY` is 20 (raised from 5) so the variance estimate behind these floors is stable enough not to flag ordinary ~5-10% fluctuations as spikes.
