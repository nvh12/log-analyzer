...
def _flow_features(scenario: SimulationScenario, source_port: int, dest_port: int) -> dict[str, float]:
    """Returns the 45 CICFlowMeter features for a simulated flow record.

    Feature names and units match flow/feature_cols.json in MinIO exactly.
    IAT values are in microseconds (μs), consistent with CICFlowMeter output.
    """
    if scenario in (SimulationScenario.NORMAL, SimulationScenario.TRAFFIC_SPIKE):
        ...
        fwd_pkt_max  = _rng(200, 1460)
        fwd_pkt_min  = _rng(20, 80)
        fwd_pkt_mean = _rng(100, 700)
        bwd_pkt_min  = _rng(20, 80)
        bwd_pkt_std  = _rng(30, 250)
        flow_bytes_s = _rng(1_000, 50_000)
        flow_pkts_s  = _rng(2, 80)
        iat_mean     = _rng(10_000, 200_000)   # 10ms–200ms
        ...
        # High-volume one-directional flood: near-zero IAT, few/no backward packets.
        ...
        # Short repeated TCP connections to SSH/FTP port; balanced but few packets.
        protocol    = 6
        ...
        iat_mean     = _rng(50_000, 300_000)   # 50ms–300ms
        ...
        min_pkt      = _rng(20, 54)
        pkt_std      = _rng(15, 80)
        fin_flag     = random.randint(1, 2)    # clean teardown on auth failure
    ...
    else:  # WEB_ATTACK — HTTP-like but with larger attack payloads
        protocol    = 6
        ...
        iat_mean     = _rng(5_000, 100_000)    # 5ms–100ms
...
def generate(scenario: SimulationScenario, log_type: LogType, target_ip: str = "10.0.0.100") -> RawLog:
    if log_type == LogType.HTTP:
        return _generate_http(scenario, target_ip)
    if log_type == LogType.FLOW:
        return _generate_flow(scenario, target_ip)
    # MIXED — coin flip
